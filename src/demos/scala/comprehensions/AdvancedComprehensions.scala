package demos.scala.comprehensions

object AdvancedComprehensions {
  private def demo1 {
  ...
  private def demo5 {
    val results = for (x <- 1 to 3; y <- 'a' to 'c') yield (x,y)
    print("Demo 5: ")
    for (item <- results) {
  ...
  private def demo6 {
    val results = for (x <- 1 to 3; y <- 10 to 12) yield x * y
    print("Demo 6: ")
    for (x <- results) {
      printf("%d ", x)
    }
