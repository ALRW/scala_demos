...
object Program {
  def buildMap(): Map[String,Employee] = {
  ...
  def printMap(title : String, data : Map[String, Employee]) {
      println("\n---" + title + "---")
  ...
  def main(args : Array[String]) {
    ...
    val result1 = myMap.filterKeys(k => k.startsWith("A") || k.startsWith("E"))
    ...
    val result3 = myMap.mapValues( emp => new Employee(emp.name + " Jones", emp.age, emp.salary))
